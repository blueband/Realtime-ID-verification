...
include_once('user_parse.php');
//  Checking some parameter in th URL
$urlList = parse_url($protocol, PHP_URL_QUERY);
list($key, $username) = userdata_parse($urlList);
...
$qrUser ="SELECT loginID FROM `qr_loginusers` WHERE username='$username'";
$sql_run =mysqli_query($con, $qrUser) or die(mysql_error());
$row = mysqli_fetch_array($sql_run);
$loginID = $row['loginID'];

// Retrieve User detail from User table 
$qrUser ="SELECT * FROM `qr_users` WHERE userloginID='$loginID'";
$sql_run =mysqli_query($con, $qrUser) or die(mysql_error());
$row = mysqli_fetch_array($sql_run);
...
$ImageName =  str_replace('/', '_', $username);
             ...
             <div class="row justify-content-center">
                 <div class="col-lg-8">
                     <div class="section-title text-center">
                         <h2 class="title">About ' . $firstname .'</h2>
                     </div> <!-- section title -->
                 </div>
             </div> <!-- row -->
             <div class="row">
